...
void CatTab(const char* table[static 256]) { table['\t'] = "^I"; }
...
void CatSetNonPrintable(const char* table[static 256]) {
  ...
  memcpy(&table[9 + 2], sample2, sizeof sample2);  //+ 2 это пропуск \n и \t
...
void Cat(int argc, char* argv[], Flags flags,
         const char* table[static 256]) {  //чтение с файла
...
int CheckingForManualInput(int argc, char* argv[]) {
  int nameFile = 0;
  for (char **name = &argv[1], **end = &argv[argc]; name != end; ++name) {
    if (**name != '-') ++nameFile;
  }
  return nameFile;
}